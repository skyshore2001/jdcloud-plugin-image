...
class ScriptEnv
{
	protected $allowedCallInScript = ["if", "else", "while", "for", "function", "use", "logit", "intval", "doubleval", "floor", "ceil", "round", 
		...
		"substr", "strlen", "strpos", "mb_substr", "mb_strlen", "mb_strpos"
	];
	function execScript($script_, $env) {
		try {
			// 安全检查
			$allowedCall = $this->allowedCallInScript;
			preg_replace_callback('/(\$_\w+)|\b(GLOBALS|global|function|class|new)\b|->\w+|(\w+)\s*\(|(\/\*)/iu', function ($ms) use ($allowedCall) {
				if (@$ms[1])
					throw new Exception("var `{$ms[1]}' is NOT allowed");
				else if (@$ms[2])
					throw new Exception("`{$ms[2]}' is NOT allowed");
				else if (@$ms[3] && !in_array($ms[3], $allowedCall))
